...
# 테스트에 사용할 데이터 및 모델 경로
TEST_DATA_DIR="/home/psw/nnUNet/data/nnUNet_raw/ToothFairy_FnPnS_sample"
MODEL_DIR="/home/psw/nnUNet/data/nnUNet_results/ToothFairy_Final/nnUNetTrainer__nnUNetResEncUNetLPlans_torchres__3d_fullres"
...
run_docker_container() {
    ...
    docker run --rm \
        --name "toothfairy3-multiinstance-algorithm" \
        --user "$(id -u):$(id -g)" \
        -e HOME=/home/user \
        ...
        "$@" # 함수로 전달된 추가 인자(GPU 옵션 등)를 여기에 추가
}
